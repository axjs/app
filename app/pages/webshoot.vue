<template>
  <f7-page>
    ...
    <!-- Search bar -->
    <f7-searchbar cancel-link="Отмена"
    ...
    <f7-list media-list
      ...
      <template v-for="item in sorted">
        <f7-list-item v-if="item.url"
                      ...
                      :key="item['.key']"
                      :title="item.title"
                      :badge="formatBytes(item.totalBytes) || item['counter'] || 0"
                      badge-color="green"
                      :subtitle="item.url">
          ...
          <f7-swipeout-actions right>
            <f7-swipeout-button color="green"
                                @click="copyItem(item)">{{item['counter'] || formatBytes(item.totalBytes)}}</f7-swipeout-button>
          </f7-swipeout-actions>
        </f7-list-item>
        <f7-list-item v-else
                      :media="icon(item)"
    ...
    <f7-fab-speed-dial>
      <!-- Actions -->
      <f7-fab-actions>
        <f7-fab-action color="pink"
                       @click="addKey">
          <f7-icon icon="icon-plus"
                   @click="addKey">A</f7-icon>
        </f7-fab-action>
  ...
  export default {
    ...
    methods: {
      ...
      clicked: function (item) {
        ...
        window.firebase.database().ref(this.key + '/' + item['.key']).child('counter').set(++counter)
      ...
      removeItem: function (item) {
        ...
        this.$f7.confirm('Are you sure to delete ?<br><br>' + item.title, 'Firebase', function () {
          window.firebase.storage().ref().child('ws/' + item['.key']).delete().then(function () {
