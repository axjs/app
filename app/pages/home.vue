...
<template>
  <f7-page>
    ...
    <f7-list>
      ...
      <f7-list-item title="Select Color"
                    :link="'/f7' + $root.theme + '/color-themes/'"
                    media="<i class='f7-icons'>keyboard_fill</i>" />
      <f7-list-item title="Show Statusbar"
                    media="<i class='f7-icons'>delete</i>"
                    v-if="$root.appMode==='native'">
        <div slot="after">
      ...
      <f7-list-item title="Change text color"
                    @click="$root.statusbarTextColor=$root.statusbarTextColor==='white'?'black':'white'"
                    v-if="$root.$f7.device.ios && $root.appMode === 'native' && $root.statusbarVisibility"
                    link="#"
                    media="<i class='f7-icons'>delete</i>" />
    </f7-list>
    ...
    <f7-list>
      ...
      <f7-list-item link="/f7ios/index/"
                    ...
                    v-if="$root.theme === 'ios'" />
      <f7-list-item link="/f7material/index/"
                    title="Material Components"
                    media="<i class='icon icon-f7' />"
                    v-if="$root.theme === 'material'" />
      <f7-list-item link="/firebase-public/"
                    title="Realtime Database"
                    :media="'<img src=\'' + images.firebase + '\' width=\'29\' />'" />
      <f7-list-item link="/firebase-private/"
                    title="Authentication & Storage"
                    :media="'<img src=\'' + images.firebase + '\' width=\'29\' />'" />
      <f7-list-item link="/state-restoration/"
                    title="App State Restoration"
                    media="<i class='f7-icons'>refresh</i>" />
      <f7-list-item link="/flexible-routing/blog/123/post/456/?display=summary#gotoend"
                    title="Flexible Routing"
                    media="<i class='f7-icons'>forward</i>" />
    ...
    <f7-block style="text-align: center">
      <f7-link href="http://app-framework.com"
               external>
        <f7-icon material="open_in_browser"
                 size="18"
